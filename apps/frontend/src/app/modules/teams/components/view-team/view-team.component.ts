...
export class ViewTeamComponent implements OnInit, OnDestroy {
  ...
  getDatesOfMonth(): Array<Date> {
    ...
    const dates = [];
    for (let day = 1; day <= lastDayOfMonth; day++) dates.push(new Date(this.year, this.month - 1, day));
  ...
  onSubmit(): void {
    ...
    if (!this.currentUserId) console.error('Cannot send calendar data: unknown user id');
